...
const auth_1 = require("../middleware/auth"); // 添加这个导入
...
router.post('/refresh', async (req, res) => {
    try {
        ...
        // 生成新token
        const newToken = jsonwebtoken_1.default.sign({ userId: user._id, role: user.role }, process.env.JWT_SECRET, { expiresIn: '7d' });
        res.json({
            success: true,
            data: { token: newToken }
        });
...
router.post('/test-login', async (req, res) => {
    try {
        ...
        // 直接返回一个测试token
        const testToken = jsonwebtoken_1.default.sign({ userId: 'test123', role: 'merchant' }, process.env.JWT_SECRET, { expiresIn: '7d' });
        res.json({
            success: true,
            message: '测试登录成功',
            data: {
